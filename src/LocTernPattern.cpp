...
{
	// Look-up Table for Spectral Clustering Result with 8 predefined neighbors: get 6561 values into 256 bins
	...
	float alpha = 0.3; // adjusts threshold, alpha = [0,1]
	...
	for(int i=1;i<img_gray.rows-1;i++) {
		for(int j=1;j<img_gray.cols-1;j++) {
			...
			// compute LTP value
			int ltp_result = 0;
			if (img_gray.at<float>(i-1,j-1) > threshold){ltp_result += 2*1;}
			if (img_gray.at<float>(i-1,j) > threshold){ltp_result += 2*3;}
			if (img_gray.at<float>(i-1,j+1) > threshold) {ltp_result += 2*9;}
			if (img_gray.at<float>(i,j+1) > threshold){ltp_result += 2*27;}
			if (img_gray.at<float>(i+1,j+1) > threshold){ltp_result += 2*81;}
			if (img_gray.at<float>(i+1,j) > threshold){ltp_result += 2*243;}
			if (img_gray.at<float>(i+1,j-1) > threshold){ltp_result += 2*729;}
			if (img_gray.at<float>(i,j-1) > threshold){ltp_result += 2*2187;}

			if (abs(img_gray.at<float>(i-1,j-1) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 1;}
			if (abs(img_gray.at<float>(i-1,j) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 3;}
			if (abs(img_gray.at<float>(i-1,j+1) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 9;}
			if (abs(img_gray.at<float>(i,j+1) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 27;}
			if (abs(img_gray.at<float>(i+1,j+1) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 81;}
			if (abs(img_gray.at<float>(i+1,j) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 243;}
			if (abs(img_gray.at<float>(i+1,j-1) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 729;}
			if (abs(img_gray.at<float>(i,j-1) - img_gray.at<float>(i,j)) < threshold2) {ltp_result += 2187;}

			// map tertiary system into decimal system using predefined look-up table		
